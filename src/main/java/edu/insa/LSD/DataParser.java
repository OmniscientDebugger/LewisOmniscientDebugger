...
public class DataParser {
    ...
    public static DataPattern parse(String s, boolean countp) {
	inputString = s;
	inputIndex = 0;
	...
	if (c == '#') {					// parse class: "#Demo" -> class Demo
	    ...
	    Class cl;
	    try {cl=Debugger.classLoader.loadClass(cName);}
	    catch (Exception e) {throw new LSDException("Not a legal class name: \""+cName+"\"");}
	    catch (Error e) {throw new LSDException("Not a legal class name: \""+cName+"\"");}
	...
	String[] inputLines = {"port = enter & parameters = P",
			       ...
			       "thisObjectClass <> #edu.insa.LSD.DataParser",
			       "VAR < VAR",
			       "a0 < VAR",
			       ""};
